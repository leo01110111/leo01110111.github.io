...
However, if you just say "Build me a neural network to predict digits" then the network would rely on its prior knowledge. Most likely using PyTorch because it's taking whatever design decisions are the most conventional. 
...
Therefore, the work of a good engineer using agents is to know at what level of granularity you need to specify things. This depends on your situation. If I'm an ML researcher testing out a new reinforcement learning algorithm, I would give Claude the algorithm pseudocode and it can figure out the rest. However, if I were building the inference stack for a self-driving car, then I would closely examine every line of code, perhaps even optimizing how the registers are used at the assembly level.  
...
LLMs can't be creative. They cannot generate out of distribution data. They also cannot choose their own reward functions. Move 37 happened in a game with rules humans invented. True creatives make their own rules. These are the reasons why I believe valuable intellectual work will continue to be led by humans and not AI.
...
Steve Jobs identified himself more as an artist than an engineer. He cared less about whether the iPhone used x86 or ARM than about what the iPhone felt like in the user’s hand. Then he communicated that vision to an army of engineers.  
